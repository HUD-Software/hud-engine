...
function(add_hudengine_cxx_linker_shared_options target)
	...
	set(MSVC_HUDENGINE_LINKER_OPTIONS
		/NOLOGO # Prevents display of the copyright message and version number
		/WX # Treat Linker Warnings as Errors
		/WINMD:NO # The linker generates only the binary executable file, but not a .winmd file
		...
		$<$<CONFIG:Debug>:
			...
			/DEBUG:FASTLINK # Leaves private symbol information in the individual compilation products used to build the executable.  Can link from two to four times as fast as full PDB generation, and is recommended when you are debugging locally and have the build products available
	...
	set(CLANG_HUDENGINE_LINKER_OPTIONS
		-NOLOGO # Prevents display of the copyright message and version number
		-WX # Treat Linker Warnings as Errors
		-DYNAMICBASE # Specifies whether to generate an executable image that can be randomly rebased at load time by using the address space layout randomization (ASLR) feature of Windows that was first available in Windows Vista
		-NXCOMPAT # Indicates that an executable is compatible with the Windows Data Execution Prevention feature
		...
		$<$<CONFIG:Debug>:
			/INCREMENTAL # Enable incremental linking
			/DEBUG:FULL # (FASTLINK is not supported by ldd-link) Moves all private symbol information from individual compilation products (object files and libraries) into a single PDB, and can be the most time-consuming part of the link. However, the full PDB can be used to debug the executable when no other build products are available, such as when the executable is deployed
			/OPT:NOREF # Keeps functions and data that are never referenced
			/OPT:NOICF # Disable COMDAT folding
		>
	...
	target_link_options(
		${target}
		PRIVATE 
		$<$<CXX_COMPILER_ID:MSVC>:${MSVC_HUDENGINE_LINKER_OPTIONS}>
		$<$<CXX_COMPILER_ID:Clang>:${CLANG_HUDENGINE_LINKER_OPTIONS}>
	)
endfunction()
