...
TEST(UniquePointer, assign_by_move_different_type) {
    const auto test = []() {
        ...
        DeleterType other_deleter;
        DeleterType deleter;
        hud::UniquePointer<HUD_TEST::NonBitwiseType2, DeleterType> other(pi, other_deleter);
        hud::UniquePointer<HUD_TEST::NonBitwiseType, DeleterType> p(nullptr, deleter);
        p = hud::move(other);
        return std::tuple{
            other.pointer() == nullptr,
...
TEST(UniquePointer, assign_by_move_different_type_different_deleter) {
    const auto test = []() {
        ...
        DeleterType2 other_deleter;
        DeleterType deleter;
        hud::UniquePointer<HUD_TEST::NonBitwiseType2, DeleterType2> other(pi, other_deleter);
        hud::UniquePointer<HUD_TEST::NonBitwiseType, DeleterType> p(nullptr, deleter);
        p = hud::move(other);
        return std::tuple{
            other.pointer() == nullptr,
