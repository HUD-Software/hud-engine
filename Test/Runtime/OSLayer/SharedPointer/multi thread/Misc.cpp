...
TEST(SharedPointer_safe, single_thread_pointer_return_object) {
    ...
    const auto test = []() {
        ...
        SharedPointer<HUD_TEST::NonBitwiseType, EThreadSafety::safe> shared_ptr(ptr);
        return std::tuple{
...
TEST(SharedPointer_safe, single_thread_is_owning) {
    ...
    const auto test = []() {
        SharedPointer<HUD_TEST::NonBitwiseType, EThreadSafety::safe> shared_ptr(new HUD_TEST::NonBitwiseType(15, nullptr));
        SharedPointer<HUD_TEST::NonBitwiseType, EThreadSafety::safe> empty_shared_ptr;
        return std::tuple{
...
TEST(SharedPointer_safe, reset_to_different_type) {
    ...
    const auto test = []() {
        ...
        auto ptr = new HUD_TEST::NonBitwiseType(15, &dtor_count);
        SharedPointer<const HUD_TEST::NonBitwiseType, EThreadSafety::safe> shared_ptr(ptr);
        const bool is_pointer_correct = shared_ptr.pointer() == ptr;

        SharedPointer<const HUD_TEST::NonBitwiseType, EThreadSafety::safe> empty_shared_ptr;
        const bool is_pointer_empty_correct = empty_shared_ptr.pointer() == nullptr;
...
TEST(SharedPointer_safe, reset_to_empty_should_not_delete_if_other_reference_exists) {
    ...
    const auto test = []() {
        ...
        SharedPointer<HUD_TEST::NonBitwiseType, EThreadSafety::safe> shared_ptr(ptr);
        const bool is_pointer_correct = shared_ptr.pointer() == ptr;
        SharedPointer<HUD_TEST::NonBitwiseType, EThreadSafety::safe> second_shared_ptr(shared_ptr);
...
TEST(SharedPointer_safe, make_shared) {
    ...
    const auto test = []() {
        ...
        i32 dtor_count = 0;
        const auto shared_ptr_2 = make_shared<HUD_TEST::NonBitwiseType, EThreadSafety::safe>(123, &dtor_count);
